...
- Verification objectives use unqualified requirement names (`verify requirement coverFloor;`) with `private import SystemRequirements::*;` — resolved correctly in Spec42 0.30+.
...
- Add small Spec42 regression fixtures for `S42-LIM-002`, `S42-LIM-005`, and `S42-LIM-007`; those have compact reproductions and would prevent regressions once fixed. (`S42-LIM-001` is covered by `cross_package_verify_requirement_resolves_via_import` in Spec42.)
